<template>
    ...
    <div
        v-else
        class="card tao-card tao-card--contract-info">

        <div
            v-if="data.length == 0"
            class="tao-empty">
            <i class="fa fa-book tao-empty__icon"/>
            <p class="tao-empty__description">Can not read this smart contract</p>
        </div>

        <div class="tao-card__header mb-3">
        ...
        <table class="tao-card__table tao-contract-info">
            <tr
                v-for="(func, index) in data"
                :key="index">
                <td :class="`tao-contract-info__func ${func.inputs.length ? 'with-input' : 'no-input'}`">
                    <span class="tao-contract-info__name">
                    ...
                    <span
                        v-if="typeof func.result !== 'undefined' && ! func.inputs.length"
                        class="tao-contract-info__result">
                        <nuxt-link
                        ...
                        <em class="tao-contract-info__type">{{ func.outputs[0].type }}</em>
                    </span>
                    <div
                        v-if="func.inputs.length"
                        class="tao-contract-info__inputs">
                        <input
                    ...
                    <div
                        ...
                        class="tao-contract-info__outputs">
                        <i class="fa fa-angle-double-right"/>
                        <span
                            v-for="(output, idx2) in func.outputs"
                            :key="idx2">
                            <span class="tao-contract-info__param-name">{{ output.name }}</span>
                            <em class="tao-contract-info__type">{{ output.type }}</em>
    ...
    methods: {
        ...
        async callFunction (functionName, signature, inputElement, outputElement) {
            ...
            document.getElementById(outputElement).innerHTML =
                `<div class="tao-contract-info__response">
        ...
        formatOuputs (output) {
            ...
            for (let i = 0; i < output.length; i++) {
                ...
                response += `<strong>${output[i].name}</strong>&nbsp;
                    <em class="tao-contract-info__type">${output[i].type}</em> :
        ...
        resetInput (e) {
            e.preventDefault()
            let inputs = document.querySelectorAll('.tao-contract-info input')
            let ouputs = document.querySelectorAll('.tao-contract-info__func.with-input .tao-contract-info__result')
