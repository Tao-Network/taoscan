<template>
    ...
    <section v-else>
        ...
        <div
            v-if="total == 0"
            class="tao-empty">
            <i class="fa fa-binoculars tao-empty__icon"/>
            <p class="tao-empty__description">No event found</p>
        </div>

        <p
            v-if="total > 0"
            class="tao-total-items">{{ _nFormatNumber('item', 'items', total) }}</p>

        <table-base
            v-if="total > 0"
            :fields="fields"
            :items="items"
            class="tao-table--events">
